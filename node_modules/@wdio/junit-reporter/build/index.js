...
class JunitReporter extends _reporter.default {
  ...
  getStandardOutput(test) {
    ...
    test.output.forEach(data => {
      switch (data.type) {
        case 'command':
          standardOutput.push(data.method ? `COMMAND: ${data.method.toUpperCase()} ` + `${data.endpoint.replace(':sessionId', data.sessionId)} - ${this.format(data.body)}` : `COMMAND: ${data.command} - ${this.format(data.params)}`);
          break;
